...
func TestMicroHuahuaIssuance(t *testing.T) {
	input := CreateTestInput(t)

	initialSupply := input.BankKeeper.GetSupply(input.Ctx, core.MicroHuahuaDenom)
	...
	for i := int64(0); i < 10; i++ {
		...
		require.Equal(t, initialSupply.Amount.Add(sdk.NewInt(i)), input.TreasuryKeeper.GetEpochInitialIssuance(input.Ctx).AmountOf(core.MicroHuahuaDenom))

		input.BankKeeper.MintCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, sdk.OneInt())))
	}
}
...
func TestPeekEpochSeigniorage(t *testing.T) {
	...
	for i := int64(0); i < 10; i++ {
		...
		err := input.BankKeeper.MintCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, issueAmount)))
		require.NoError(t, err)

		burnAmount := sdk.NewInt(rand.Int63()%(faucetBalance.Amount.Int64()+issueAmount.Int64()) + 1)
		err = input.BankKeeper.BurnCoins(input.Ctx, faucetAccountName, sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, burnAmount)))
		require.NoError(t, err)
