...
//
// Computes important economic indicators for the stability of Chihuahua currencies.
//
...
// - TSL: Total Staked Huahua
// - TRL: Computes the Tax Reward per unit Huahua (TR/TSL)
...
// 	// Compute Total Staked Huahua (TSL)
// 	totalStakedHuahua := k.stakingKeeper.TotalBondedTokens(ctx)

// 	k.SetTSL(ctx, epoch, totalStakedHuahua)
...
// 	seigniorageRewards := sdk.DecCoins{sdk.NewDecCoinFromDec(core.MicroHuahuaDenom, seigniorageRewardsAmt)}
// 	SR := k.alignCoins(ctx, seigniorageRewards, core.MicroSDRDenom)

// 	k.SetSR(ctx, epoch, SR)
// }

// TRL returns Tax Rewards per Huahua for the epoch
