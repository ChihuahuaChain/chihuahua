...
func (m *EpochState) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	_ = l
	{
		size := m.TotalStakedHuahua.Size()
		i -= size
		if _, err := m.TotalStakedHuahua.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
		i = encodeVarintGenesis(dAtA, i, uint64(size))
	}
...
func (m *EpochState) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TotalStakedHuahua", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.TotalStakedHuahua.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
