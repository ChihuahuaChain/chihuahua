...
var (
	...
	KeyPoolPermissionedCreatorAddresses = []byte("PoolPermissionedCreatorAddresses")
)
...
func (p *Params) ParamSetPairs() paramstypes.ParamSetPairs {
	return paramstypes.ParamSetPairs{
		...
		paramstypes.NewParamSetPair(KeyPoolPermissionedCreatorAddresses, &p.PoolPermissionedCreatorAddresses, validatePoolPermissionedCreatorAddresses),
	}
}
...
func (p Params) Validate() error {
	for _, v := range []struct {
		...
		{p.PoolPermissionedCreatorAddresses, validatePoolPermissionedCreatorAddresses},
	} {
...
func validateBuildersAddresses(i interface{}) error {
	...
	for i, w := range v {
		// Every builder address must be a valid bech32 account address.
		// An empty/invalid address used to be accepted here (with the intent of
		// routing that share to the community pool), but the distribution path
		// in SendAmountToBuilders never implemented that and instead called
		// sdk.MustAccAddressFromBech32, which panics on an empty/invalid string.
		// Because builder commissions are paid during the liquidity batch
		// EndBlocker, such a panic would halt the chain. Rejecting invalid
		// addresses here makes it impossible to persist a chain-halting param.
		if _, err := sdk.AccAddressFromBech32(w.Address); err != nil {
			return fmt.Errorf("invalid builder address at index %d: %w", i, err)
		}
...
func validatePoolPermissionedCreatorAddresses(i interface{}) error {
	v, ok := i.([]string)
	if !ok {
		return fmt.Errorf("invalid parameter type: %T", i)
	}
