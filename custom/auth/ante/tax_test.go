...
import (
	...
	"github.com/ChihuahuaChain/chihuahua/custom/auth/ante"
	core "github.com/ChihuahuaChain/chihuahua/types"
	// markettypes "github.com/ChihuahuaChain/chihuahua/x/market/types"
	// wasmtypes "github.com/ChihuahuaChain/chihuahua/x/wasm/types"
)
...
func (suite *AnteTestSuite) TestEnsureMempoolFeesSwapSend() {
	...
	// keys and addresses
	priv1, _, _ /*addr1*/ := testdata.KeyTestPubAddr()
...
func (suite *AnteTestSuite) TestEnsureMempoolFeesSendHuahuaTax() {
	suite.SetupTest(true) // setup
	suite.txBuilder = suite.clientCtx.TxConfig.NewTxBuilder()
	...
	// msg and signatures
	sendAmount := int64(1000000)
	sendCoins := sdk.NewCoins(sdk.NewInt64Coin(core.MicroHuahuaDenom, sendAmount))
	msg := banktypes.NewMsgSend(addr1, addr1, sendCoins)
	...
	// Huahua must pass with tax before the specified tax block height
	_, err = antehandler(suite.ctx, tx, false)
	...
	tk := suite.app.TreasuryKeeper
	expectedTax := tk.GetTaxRate(suite.ctx).MulInt64(sendAmount).TruncateInt()
	if taxCap := tk.GetTaxCap(suite.ctx, core.MicroHuahuaDenom); expectedTax.GT(taxCap) {
		expectedTax = taxCap
	}

	// set tax amount
	suite.txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, expectedTax)))
	tx, err = suite.CreateTestTx(privs, accNums, accSeqs, suite.ctx.ChainID())
	suite.Require().NoError(err)
...
func (suite *AnteTestSuite) TestEnsureMempoolFeesSwapSendHuahuaTax() {
	suite.SetupTest(true) // setup
	suite.txBuilder = suite.clientCtx.TxConfig.NewTxBuilder()
	...
	// msg and signatures
	sendAmount := int64(1000000)
	//sendCoin := sdk.NewInt64Coin(core.MicroHuahuaDenom, sendAmount)
	//msg := markettypes.NewMsgSwapSend(addr1, addr1, sendCoin, core.MicroKRWDenom)
...
func (suite *AnteTestSuite) TestEnsureMempoolFeesMultiSendHuahuaTax() {
	suite.SetupTest(true) // setup
	suite.txBuilder = suite.clientCtx.TxConfig.NewTxBuilder()
	...
	// must pass with tax
	suite.txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, expectedTax.Add(expectedTax))))
	tx, err = suite.CreateTestTx(privs, accNums, accSeqs, suite.ctx.ChainID())
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().NoError(err, "Decorator should not have errored on fee higher than local gasPrice")
}

func (suite *AnteTestSuite) TestEnsureMempoolFeesInstantiateContractHuahuaTax() {
...
func (suite *AnteTestSuite) TestEnsureMempoolFeesExecuteContractHuahuaTax() {
...
func (suite *AnteTestSuite) TestEnsureMempoolFeesExecHuahuaTax() {
