...
depending on the Chihuahua codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
Chihuahua version to Docker containers during the initialization. This
design allows for the opportunity of testing chain upgrades in the
future by providing an older Chihuahua version to the container,
...
1. Chain Initialization
    ...
    of the previous Chihuahua version, inside `configurer/upgrade.go`. This is
    ...
    current branch version of Chihuahua codebase
    ...
    with IBC initialized at the previous Chihuahua version

    - If none are true, we only need one chain at the current branch version
    of the Chihuahua code
...
2. Setting up e2e components
    ...
    - IBC testing
        ...
        - Upgrade Testing
        - 2 chains of the older Chihuahua version are created, and
    ...
    - Upgrade testing
        ...
        - Current branch Chihuahua version is spun up to continue with testing
...
such as the `app.toml`. This package directly depends on the Chihuahua
codebase.
...
production chihuahua container to start the `chihuahuad` service.
...
### To build the debug Chihuahua image
...
- `HUA_E2E_SKIP_UPGRADE` - when true, skips the upgrade tests.
If HUA_E2E_SKIP_IBC is true, this must also be set to true because upgrade
tests require IBC logic.

- `HUA_E2E_SKIP_IBC` - when true, skips the IBC tests tests.

- `HUA_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `HUA_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `HUA_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `HUA_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `HUA_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
...
```json
{
    ...
    "buildFlags": "-tags e2e",
    "env": {
        "HUA_E2E_SKIP_IBC": "true",
        "HUA_E2E_SKIP_UPGRADE": "true",
        "HUA_E2E_SKIP_CLEANUP": "true",
        "HUA_E2E_SKIP_STATE_SYNC": "true",
        "HUA_E2E_UPGRADE_VERSION": "v11",
        "HUA_E2E_DEBUG_LOG": "true",
        "HUA_E2E_FORK_HEIGHT": "42345124" 
    }
}
```
