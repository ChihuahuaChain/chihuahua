...
func NewManager(isUpgrade bool, isFork bool, isDebugLogEnabled bool) (docker *Manager, err error) {
	...
	docker.network, err = docker.pool.CreateNetwork("chihuahua-testnet")
	if err != nil {
		return nil, err
	}
	return docker, nil
...
func (m *Manager) ExecTxCmdWithSuccessString(t *testing.T, chainID string, containerName string, command []string, successStr string) (bytes.Buffer, bytes.Buffer, error) {
	allTxArgs := []string{fmt.Sprintf("--chain-id=%s", chainID), "-b=block", "--yes", "--keyring-backend=test", "--output=json", "--log_format=json", "--gas-prices=0.1uchihuahua", "--gas-adjustment=1.5"}
...
func (m *Manager) RunHermesResource(chainAID, chihuahuaARelayerNodeName, chihuahuaAValMnemonic, chainBID, chihuahuaBRelayerNodeName, chihuahuaBValMnemonic string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("HUA_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("HUA_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("HUA_A_E2E_VAL_MNEMONIC=%s", chihuahuaAValMnemonic),
				fmt.Sprintf("HUA_B_E2E_VAL_MNEMONIC=%s", chihuahuaBValMnemonic),
				fmt.Sprintf("HUA_A_E2E_VAL_HOST=%s", chihuahuaARelayerNodeName),
				fmt.Sprintf("HUA_B_E2E_VAL_HOST=%s", chihuahuaBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
	...
	pricefeederResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Mounts: []string{
				...
				fmt.Sprintf("%s:/.chihuahuad", valCondifDir),
			},
			ExposedPorts: []string{
				"7171",
			},
...
func (m *Manager) RunNodeResource(chainID string, containerName, valCondifDir string) (*dockertest.Resource, error) {
	...
	runOpts := &dockertest.RunOptions{
		Name:       containerName,
		Repository: m.ChihuahuaRepository,
		Tag:        m.ChihuahuaTag,
		NetworkID:  m.network.Network.ID,
		User:       "root:root",
		Cmd:        []string{"start", "--trace"},
		Mounts: []string{
			fmt.Sprintf("%s/:/chihuahua/.chihuahuad", valCondifDir),
			fmt.Sprintf("%s/scripts:/chihuahua", pwd),
		},
	}
