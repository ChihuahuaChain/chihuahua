...
func NewDecodeStore(cdc codec.Codec) func(kvA, kvB kv.Pair) string {
	return func(kvA, kvB kv.Pair) string {
		...
		case bytes.Equal(kvA.Key[:1], types.TSLKey):
			var TotalStakedHuahuaA, TotalStakedHuahuaB sdk.IntProto
			cdc.MustUnmarshal(kvA.Value, &TotalStakedHuahuaA)
			cdc.MustUnmarshal(kvB.Value, &TotalStakedHuahuaB)
			return fmt.Sprintf("%v\n%v", TotalStakedHuahuaA, TotalStakedHuahuaB)
		default:
