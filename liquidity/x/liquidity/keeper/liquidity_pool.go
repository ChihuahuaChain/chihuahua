...
func (k Keeper) SendAmountToBuilders(params types.Params, from sdk.AccAddress, totalCommission sdk.Coin, sendCoin func(from, to sdk.AccAddress, coin sdk.Coin) error) (sdk.Coin, error) {
	...
	if len(params.BuildersAddresses) > 0 {
		...
		for _, builderAddr := range params.BuildersAddresses {
			...
			// Never panic inside this path: it runs during the liquidity batch
			// EndBlocker, so a panic would halt the chain. Param validation
			// already rejects invalid builder addresses, but we convert safely
			// here as defense in depth.
			builderAcc, err := sdk.AccAddressFromBech32(builderAddr.Address)
			if err != nil {
				return sdk.Coin{}, sdkerrors.Wrapf(err, "invalid builder address %q", builderAddr.Address)
			}
