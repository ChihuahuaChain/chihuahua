...
func (k Keeper) SettleSeigniorage(ctx sdk.Context) {
	...
	seigniorageHuahuaAmt := k.PeekEpochSeigniorage(ctx)
	if seigniorageHuahuaAmt.LTE(sdk.ZeroInt()) {
		return
	}
	...
	seigniorageDecCoin := sdk.NewDecCoin(core.MicroHuahuaDenom, seigniorageHuahuaAmt)
	...
	burnCoins := sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, burnAmt))
	...
	leftCoins := sdk.NewCoins(sdk.NewCoin(core.MicroHuahuaDenom, leftAmt))
