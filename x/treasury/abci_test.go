...
func TestEndBlockerIssuanceUpdateWithBurnModule(t *testing.T) {
	...
	supply := input.BankKeeper.GetSupply(input.Ctx, core.MicroHuahuaDenom)
	...
	require.Equal(t,
		...
		supply.Amount.Sub(keeper.InitCoins.AmountOf(core.MicroHuahuaDenom)),
		issuance.AmountOf(core.MicroHuahuaDenom))
}
